// strategy-(autorun).js
// Поиск трендовой фазы в окне рабочего дня
// точки входа - в сторону тренда на коррекции от границ канала.
...
window.StrategyDefinition = (function() {
    ...
    const moneyManagementSettings = Object.freeze({
        ...
        winPayout: 0.8,
        // Минимальный интервал между точками входа = expirationMinutes * minEntryIntervalMultiplier
...
`;
    ...
    const signalsSource = `
// === СИГНАЛЫ ===
// phaseSignal, 0..1, разрешающий множитель
// entrySignal, -1..+1, направленный краткосрочный сигнал
// trendSignal, -1..+1, медленный тренд
// validSignal, отключен, всегда 0
// resultSignal = ((entry*wEntry + trend*wTrend + valid*wValid) × phase)
...
const wEntry = 1;   // Множитель для точки входа
const wTrend = 1;   // Множитель для тренда
const wValid = 1;   // Множитель для валидного сигнала
...
if (bb0 && bb0.m > 0) {
    ...
    const WINDOW = 100;  // Долгосрочный максимум за 200 свечей
    let maxBw = bwNorm;
    ...
    relBw = Math.round(relBw * 1) / 1;
    // phaseSignal = relBw напрямую (0..1)
    phaseSignal = relBw;
}

// --- MACD: направленный сигнал ---
// Нормализуем текущую гистограмму за окно и инвертируем знак для направления входа
const mac0 = i.macd(0);
if (mac0 && mac0.histogram !== null && mac0.histogram !== undefined) {
    ...
    // --- Медленный тренд по наклону средней линии BB ---
    const bbTrendNow = i.bb(0);
    const bbTrendPast = i.bb(-20);
    if (bbTrendNow && bbTrendPast && bbTrendNow.m > 0 && bbTrendPast.m > 0) {
        let bbMidSlope = ((bbTrendNow.m - bbTrendPast.m) / bbTrendPast.m) * 100;
        bbMidSlope = Math.max(-1, Math.min(1, bbMidSlope));
        trendSignal = bbMidSlope * 3;
    }

    // --- Валидный сигнал отложен, пока оставляем 0 ---
    validSignal = 0.1;
